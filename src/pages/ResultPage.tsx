...
import { Edit, Printer, Download, RefreshCw } from 'lucide-react'
...
export default function ResultPage() {
  ...
  function handleRecalculate() {
    try {
      calculate()
      toast({ title: 'Kalkulasi diperbarui', variant: 'success' })
    } catch {
  ...
  function handleExportJSON() {
    ...
    if (project) {
      ...
      toast({ title: 'JSON berhasil diexport', variant: 'success' })
    }
  }
        ...
        <Tabs defaultValue="ringkasan" className="space-y-4">
            ...
            <TabsList className="h-auto p-1 flex-nowrap inline-flex min-w-max bg-muted gap-1">
              {[
                ...
                <TabsTrigger
                  key={value}
                  value={value}
                  className="text-sm whitespace-nowrap data-[state=active]:bg-navy data-[state=active]:text-white"
                >
                  {label}
                </TabsTrigger>
              ))}
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-5 lg:p-6">
            ...
            <TabsContent value="biaya"      className="mt-0">
              <TabStrukturBiaya results={r} inputs={currentInputs} />
            </TabsContent>

            <TabsContent value="pendapatan" className="mt-0">
              <TabProyeksiPendapatan results={r} inputs={currentInputs} />
            </TabsContent>
            ...
            <TabsContent value="bagihasil"  className="mt-0">
              <TabBagiHasil results={r} />
            </TabsContent>

            <TabsContent value="sensitivitas" className="mt-0">
