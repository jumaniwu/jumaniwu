          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0">
              <tr className="bg-navy text-white">
                {['Periode', 'Penerimaan', 'Biaya Bangun', 'Biaya Persiapan', 'Biaya Ops', 'Net CF', 'Kumulatif'].map(h => (
            ...
            <tbody className="divide-y divide-border">
              {data.map((row, i) => (
                <tr key={i} className={i % 2 === 0 ? '' : 'bg-muted/20'}>
                  ...
                  <td className="px-3 py-2 text-right font-mono text-green-700">{formatRupiah(row.penerimaan, true)}</td>
                  <td className="px-3 py-2 text-right font-mono text-red-700">({formatRupiah(row.biayaBangun, true)})</td>
                  <td className="px-3 py-2 text-right font-mono text-red-700">({formatRupiah(row.biayaPersiapan, true)})</td>
                  <td className="px-3 py-2 text-right font-mono text-red-700">({formatRupiah(row.biayaOperasional, true)})</td>
                  <td className={`px-3 py-2 text-right font-mono font-semibold ${row.netCF >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                    {formatRupiah(row.netCF, true)}
                  </td>
                  <td className={`px-3 py-2 text-right font-mono font-semibold ${row.kumulatif >= 0 ? 'text-blue-700' : 'text-red-700'}`}>
